...
// ================================================================ 1. TITLE
{
  ...
  s.addText("CNN vs MLP", { x: 0.92, y: 1.95, w: 8.4, h: 1.0, fontSize: 54, color: "2A1618",
    bold: true, fontFace: "Cambria", margin: 0 });
  s.addText("A matched, head-to-head comparison of two machine-learning approaches" + String.fromCharCode(10) + "for microwave tumor localization",
  ...
  s.addText("Preprocessing pipeline v2 (session-mean subtraction removed)",
...
{
  const s = p.addSlide();
  title(s, "Data pipeline", "Three steps from raw sweep to model input");
  s.addImage({ path: A + "pipeline_v2.png", x: 0.4, y: 1.42, w: 9.2, h: 2.28 });
  bullets(s, [
    { t: "Step 1 removes everything that is not the tumor; step 3 (per-session z-score) is the key drift defense: each session is standardized by its own statistics. Applied identically to CNN and MLP.", },
    { t: "v2 change: the per-session mean subtraction used in v1 was removed. An ablation showed it was redundant with the z-score for the MLP (up to +6 pts recovered on hard configs) and roughly neutral for the CNN outside TDR inputs.", b: true },
...
// ================================================================ 15. DIVIDER: RESULTS
divider("Part 3", "Results", "Per-position vote accuracy (%) under LOSO · identical raw inputs (mag + phase) · chance = 2–3% · pipeline v2");

// ---- results data: [phantom, nway, cnn, mlp, cnnSd, mlpSd] -----------------
const R = {
  all_full:   [["A3 Empty", 51, 99.3, 99.3, 1.1, 0.9], ["A3 + F4", 39, 97.4, 98.7, 3.0, 2.2], ["A3 + F5", 33, 100.0, 94.9, 0.0, 3.8]],
  pair_full:  [["A3 Empty", 51, 99.3, 99.3, 1.1, 0.9], ["A3 + F4", 39, 96.8, 96.8, 2.5, 2.1], ["A3 + F5", 33, 97.0, 84.9, 5.3, 6.6]],
  single:     [["A3 Empty", 51, 99.3, 99.3, 1.1, 0.9], ["A3 + F4", 39, 92.9, 90.4, 7.4, 8.0], ["A3 + F5", 33, 72.7, 64.7, 6.1, 5.7]],
  pair_phys:  [["A3 Empty", 51, 99.3, 99.3, 1.1, 0.9], ["A3 + F4", 39, 98.7, 97.4, 2.6, 2.6], ["A3 + F5", 33, 99.0, 89.9, 1.8, 7.6]],
  single_phys:[["A3 Empty", 51, 99.3, 98.7, 1.1, 1.9], ["A3 + F4", 39, 95.5, 93.6, 3.2, 4.6], ["A3 + F5", 33, 76.8, 66.7, 3.5, 8.9]],
  refl_all_raw:  [["A3 Empty", 51, 99.3, 99.3, 1.1, 0.9], ["A3 + F4", 39, 95.5, 93.6, 3.9, 3.9], ["A3 + F5", 33, 100.0, 91.9, 0.0, 3.8]],
  refl_pair_raw: [["A3 Empty", 51, 99.3, 99.3, 1.1, 0.9], ["A3 + F4", 39, 91.7, 92.3, 2.5, 3.6], ["A3 + F5", 33, 92.9, 80.8, 4.6, 7.1]],
  refl_all_phys: [["A3 Empty", 51, 99.3, 99.3, 1.1, 0.9], ["A3 + F4", 39, 96.2, 94.2, 3.3, 2.8], ["A3 + F5", 33, 100.0, 88.9, 0.0, 5.2]],
  refl_pair_phys:[["A3 Empty", 51, 99.3, 98.0, 1.1, 1.6], ["A3 + F4", 39, 94.2, 91.7, 2.5, 4.6], ["A3 + F5", 33, 98.0, 81.8, 3.5, 4.3]],
};
...
resultSlide("Results · raw inputs", "All 4 antennas: full S-parameters (16)", R.all_full,
  ...
  "With the full antenna array, both methods are near-perfect on every phantom. The CNN stays at 100% on F5; the MLP gives up ~5 points there under pipeline v2.");
// 17: all-4 DD figure (carries the legend)
ddSlide("Error Locations: all 4 antennas", "dd_all_h_v2.png",
...
resultSlide("Results · raw inputs", "All 4 antennas: reflection only (S11, S22, S33, S44)", R.refl_all_raw,
  ...
  "Dropping transmission barely touches the CNN (F5 stays at 100%), while the MLP gives up 3-5 points on the glandular phantoms. Four reflection coefficients are enough for a CNN.");
// 18b: all-4 refl DD figure
ddSlide("Error Locations: all 4 antennas, reflection only", "dd_refl_all_h_v2.png",
...
resultSlide("Results · raw inputs", "2 antennas (ports 1 & 3): full S-parameters", R.pair_full,
  ...
  "Halving the antennas separates the methods on F5: the CNN holds 97% while the MLP drops to 85%. On F4 the two are tied under pipeline v2.");
// 19: pair DD figure
ddSlide("Error Locations: 2 antennas (1 & 3)", "dd_pair_h_v2.png",
...
resultSlide("Results · raw inputs", "2 antennas (1 & 3): reflection only (S11, S33)", R.refl_pair_raw,
  ...
  "The CNN holds 92-99% while the MLP falls to 81% on F5. F4 remains the single config in the study where the MLP edges the CNN (92.3 vs 91.7).");
// 21b: pair refl DD figure
ddSlide("Error Locations: 2 antennas (1 & 3), reflection only", "dd_refl_pair_h_v2.png",
...
resultSlide("Results · raw inputs", "1 antenna: S11 only", R.single,
  ...
  "The extreme case: one antenna, one S-parameter. The CNN holds 73–99%; the MLP falls to 65% on F5. The gap grows as the task gets harder, and the F5 errors cluster inside the glandular insert (next slide).");
// 21: single DD figure
ddSlide("Error Locations: 1 antenna (S11)", "dd_single_h_v2.png",
...
// ================================================================ 24b. REFLECTION-ONLY + PHYSICS
{
  ...
  notebox(s, "With physics features, reflection-only costs the CNN almost nothing (F5: 100% with four antennas, 98% with two); the MLP pays 8-11 points. A reflection-only system is practical with a CNN.", 5.02, 0.5, 11.5);
}
...
// ================================================================ 25. PHYSICS TAKEAWAY + TDR + HIER
{
  ...
  bullets(s, [
    { t: "F5, 2 antennas:  CNN +2.0 (97→99) · MLP +5.1 (85→90)", },
    { t: "F5, 1 antenna:   CNN +4.0 (73→77) · MLP +2.0 (65→67)", },
    { t: "F5, all 4:            MLP +4.0 (95→99); CNN already at 100", },
  ...
  s.addText("TDR-only ablation (selected)", { x: 0.5, y: 3.78, w: 5.4, h: 0.32,
    fontSize: 14, bold: true, color: DARK, fontFace: "Calibri", margin: 0 });
  bullets(s, [
    { t: "F4, 1 antenna: CNN 55 / MLP 64  (vs 93 / 90 raw): a big loss for both.", },
    { t: "TDR is where v2 hurts: removing the mean subtraction leaves coherent session clutter in the envelope (CNN F5-pair tdr fell 79→63).", },
...
{
  const s = p.addSlide();
  title(s, "Pipeline comparison", "What removing the mean subtraction changed (v1 vs v2)");
  s.addText("MLP: clear gains", { x: 0.5, y: 1.35, w: 4.4, h: 0.32, fontSize: 14,
    bold: true, color: DARK, fontFace: "Calibri", margin: 0 });
  bullets(s, [
    { t: "F5 single raw:  58.6 → 64.7  (+6.1)", },
    { t: "F5 refl-pair raw:  74.8 → 80.8  (+6.1)", },
    { t: "F4 pair physics:  92.3 → 97.4  (+5.1)", },
    { t: "F5 pair physics:  85.9 → 89.9  (+4.0)", },
    { t: "F4 pair raw:  93.6 → 96.8  (+3.2)", },
    { t: "Empty phantom: unchanged (ceiling).", },
  ], { x: 0.5, y: 1.7, w: 4.4, h: 2.6, fontSize: 12.5, gap: 6 });
  s.addText("CNN: mixed, mostly neutral", { x: 5.15, y: 1.35, w: 4.4, h: 0.32, fontSize: 14,
    bold: true, color: DARK, fontFace: "Calibri", margin: 0 });
  bullets(s, [
    { t: "Most raw / physics configs: within ±1 point of v1.", },
    { t: "Single-antenna dips: F4 raw 96.8 → 92.9, F5 raw 75.8 → 72.7, F5 physics 79.8 → 76.8.", },
    { t: "TDR-only suffers badly: F5 pair 78.8 → 62.6; F4 single 84.0 → 55.1 (a fold collapse returns).", },
  ], { x: 5.15, y: 1.7, w: 4.4, h: 2.6, fontSize: 12.5, gap: 8 });
  notebox(s, "Verdict: the mean subtraction was redundant for frequency-domain inputs once the per-session z-score exists (the MLP gains by dropping it), but TDR inputs genuinely need it: coherent session clutter must be removed BEFORE the envelope is computed. Best practice going forward: skip it for raw/physics, keep it for TDR.", 4.45, 0.95, 12.5);
}
...
// ================================================================ 26. CONCLUSIONS
{
  ...
  cc(5.15, 4.05, "Fewer antennas is viable with a CNN", "2 antennas: 97–99% on every phantom. Even 1 antenna: 73–99%.");
}

// ================================================================ write
p.writeFile({ fileName: "CNN_vs_MLP_Comparison_v2_noMeanSub.pptx" }).then(() => console.log("deck written"));
