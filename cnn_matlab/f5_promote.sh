#!/bin/bash
# F5 audit-driven promotions to 100ep (dataset-wide STILL-IMPROVING flags):
# headline + the three 2-5 GHz cells. Labels get -p100 to keep 20ep results.
...
base() {
  ...
  export CNN_LOSO_ZSCORE="" CNN_LOSO_INPUTNORM="" CNN_LOSO_MAGDB="" CNN_LOSO_CURVES="1" CNN_LOSO_EPOCHS="100"
  export CNN_LOSO_ANT_MODE="all" CNN_LOSO_PORTS="1 2 3 4"
  export CNN_LOSO_SESSIONS="1317 1341 1409" CNN_LOSO_SETLABEL="f5aug-p100"
}
run() { echo "### F5-P100 $1 ###"; "$MAT" -batch "run('$SCRIPT')" 2>&1 | grep -E "LOSO position|STILL IMPROVING|rror" | head -4; }
base; run "headline full band"
