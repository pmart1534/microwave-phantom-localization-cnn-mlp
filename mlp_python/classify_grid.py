...
def run(name):
    ...
    for label, chans in ANT:
        ...
        for w in WIDTHS:
            ...
            acc = SN.knn_classify_loso(dd) * 100
        ...
        grid.append(row)
    json.dump(dict(dataset=name, center=center, nclass=nclass, chance=100/nclass,
                   widths=[str(w) for w in WIDTHS], grid=grid),
              open(B.RESULTS / f"classify_grid_{name}.json", "w"), indent=1)
    print(f"  saved classify_grid_{name}.json ({time.time()-t0:.0f}s)", flush=True)
